...
export const useWeeklyCalendarLogic = ({ 
  ...
  // 태그 총합 계산 - 메모이제이션
  const tagTotals = useState(() => calculateTagTotals(safeSchedules))[0];

  // 의존성이 변경될 때만 tagTotals 업데이트
  useEffect(() => {
    // 빈 effect로 리렌더링 방지
  }, [safeSchedules.length]); // 길이만 체크하여 불필요한 재계산 방지
  ...
  return {
    ...
    safeTags,
    safeTagItems,
    tagTotals: calculateTagTotals(safeSchedules), // 매번 계산하지만 안전하게
    repeatOptions,
    intervalOptions,
